...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    let exposeTreeProps = reactive({ treeData });
    // 选择节点类型
    ...
    const actions = {
      ...
      add(record: { key: string; }) {
        ...
        // 循环遍历设置key值
        const setKey = (count: number, index: number, parentKey: string | number) => {
    ...
    // 操作组件
    const TreeAction = (_props: { action: Function, type: string, danger?: boolean }) => {
      const { action, type, danger } = _props;
      const getTypeTitle = (type: string) => {
         let title = '';
         switch(type) {
           case 'save': title = '保存'; break;
           case 'add': title = '添加'; break;
           case 'edit': title = '编辑'; break;
    ...
    return () => (
      <div class={styles["tree-main"]}>
        <Tree
          ...
          v-slots={{
            title: (record: { key: string; title?: string; parent?: number; }) => {
              return (
                <div class={styles["slots-title"]}>
                  ...
                  <div class={styles["slots-title-action"]}>
                    {Object.is(state.selectKey, record.key) ? <TreeAction action= {()=> actions.save()} type="save" /> : (
                      <>
                        {Object.is(record.parent, 0) ? <TreeAction action= {()=> actions.add(record)} type="add" /> : (
                          <>
                            {Object.is(record.parent, level) ? (
                              <>
                                <TreeAction action= {()=> actions.edit(record)} type="edit" />
                                <a-divider type="vertical" />
                                <TreeAction action= {()=> actions.delete(record)} type="delete" danger />
                              </>
                            ) : (
                              <>
                                <TreeAction action= {()=> actions.add(record)} type="add" />
                                <a-divider type="vertical" />
                                <TreeAction action= {()=> actions.edit(record)} type="edit" />
                                <a-divider type="vertical" />
                                <TreeAction action= {()=> actions.delete(record)} type="delete" danger />
