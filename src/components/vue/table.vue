...
<template>
    <div class="table-card">
        <!-- 条件区域111 -->
        <div :class="`table-search-area ${showMore ? 'table-search-area-showMore' : ''}`"
            :style="{ height: `${tableSearchAreaHeight}px` }">
            <div class="table-search-area-search">
                <div ref="searchAreaRef">
                    <slot name="searchArea">
                        <tsx-form :renderList="searchRenderList" :formData="searchData" boxType="search"
                            @unifyEvent="handleSearch"></tsx-form>
                    </slot>
                </div>
            </div>
        ...
        <div class="table-header">
            ...
            <div class="table-header-tools" v-if="!hideTable">
                <a-tooltip>
                    ...
                    <tsx-icon :name="fullScreen ? 'outScreen' : 'screen'" @click="switchScreenDisplay" />
                </a-tooltip>
        ...
        <div class="vue-table-content" ref="vue-tableContent">
            <a-table v-if="!hideTable" :rowKey="rowKey" :loading="loading" class="tableViewer"
                :class="needStriped && 'ant-table-striped'" :row-class-name="needStriped ? (_record: any, index: number) => (index % 2 === 1 ? 'table-striped' : null) : null
                    " size="middle" :scroll="{ x: 'max-content', y: `calc(100vh - ${tableSearchAreaHeight}px - 336px)` }"
                :pagination="false" :columns="columns" :data-source="dataSource"
                :row-selection="needRowSelection ? rowSelection : null" v-model:expandedRowKeys="innerExpandedRowKeys"
                @expand="handleExpand" @expandedRowsChange="handleExpandedRowsChange">
                <template #emptyText>
                    <!-- emptyIcon -->
        ...
        <!-- 分页区域 -->
        <div class="pageArea" v-if="!hideTable">
            <span class="count">共 {{ paginationConfig.total }} 项数据</span>
            <div class="pageArea-content">
                <a-pagination :current="paginationConfig.current" :total="paginationConfig.total" :showSizeChanger="true"
                    @change="handleChangePage" :responsive="true" />
...
import {
    ...
    PageNationFace,
    rowSelectionType,
} from "@/types/mainInterface";
import { CaretDownOutlined } from "@ant-design/icons-vue";
import TsxInput from "@/components/tsx/input";
